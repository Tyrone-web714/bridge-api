...
const CAPABILITIES = Object.freeze([
  ...
  R('fleet.issue_anomaly_candidate', 'Fleet Intelligence', 'Vehicle issue/exception detection', 'D0', ['bridge-api/services/intelligenceExecution/fleetIntelligence.js','docs/implementation/fleet-intelligence-foundation/PREDICTIVE_MAINTENANCE_BOUNDARY.md','docs/implementation/fleet-intelligence-foundation/generated/fleet_issue_catalog.json'], { safety: true, deterministic: true, currentType: 'deterministic issue/exception detection based on existing authoritative Fleet Intelligence evidence', cost: 'EXTREME', rationale: 'The approved capability is deterministic issue/exception detection based on existing authoritative Fleet Intelligence evidence. No probabilistic or model-based anomaly detection is authorized. Statistical anomaly detection may only be reconsidered later if real production evidence demonstrates deterministic rules are insufficient and the owner separately approves that expansion.' }),
...
function renderDocs(registry) {
  ...
  const lines = {
    ...
    'MS001_COMPLETION_REPORT.md': [
      ...
      '',
      'MS-001 classified the existing TSR capability set by minimum appropriate execution class without selecting models or providers. D0 capabilities are excluded from unnecessary model benchmarking. D1/D2/D3 capabilities form the candidate set for the next Model Selection Gate package. No unresolved MS-001 owner-review classification remains. No new TSR intelligence domain or product capability was authorized or implemented. MS-002 is the owner-approved repository-only benchmark and acceptance framework package; it does not execute hosted benchmarks or select providers or models.'
    ].join('\n')
  };
  return lines;
}
