...
function renderDocs(registry) {
  ...
  const lines = {
    ...
    'MS001_COMPLETION_REPORT.md': [
      ...
      '',
      'MS-001 classified the existing TSR capability set by minimum appropriate execution class without selecting models or providers. D0 capabilities are excluded from unnecessary model benchmarking. D1/D2/D3 capabilities form the candidate set for the next Model Selection Gate package. No unresolved MS-001 owner-review classification remains. No new TSR intelligence domain or product capability was authorized or implemented. MS-002 remains unstarted and requires separate owner approval.'
    ].join('\n')
  };
  return lines;
}
