...
| --- | --- | --- |
| Production DB state | OWNER APPROVAL REQUIRED | Explicit approval to run read-only preflight against confirmed production `DATABASE_URL`. |
| Production backup | BLOCKED | Database provider dashboard/API/CLI access or owner-provided provider evidence. |
| Restore capability | NOT VERIFIED | Non-production restore target and provider restore access. |
