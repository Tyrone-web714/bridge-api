...
| Mobile authenticated private media | Deployment Repair In Progress | Branch `mobile-authenticated-private-media` adds authenticated private-media rendering and repairs the root mobile session/context model after physical Android testing exposed camera upload, camera-return login state, stop-scoped note retrieval, four-photo preview truncation, nondeterministic post-camera return state, photo-save non-persistence, and Driver Copilot authentication defects. Merge-gate validation passed for the backend `/api/ai` driver-auth repair, mobile session restoration, and photo persistence fixes. A Render startup failure then exposed that historical migration `004` had been modified by the Copilot permission seed; migration `004` has been restored and additive migration `011_driver_copilot_permission_repair.sql` now carries the forward permission repair. The corrected main commit must deploy successfully to Render and be physically validated on Android before Driver Copilot acceptance or public R2 shutdown can be considered for owner approval. |
