...
| Mobile authenticated private media | Ready For Physical Validation | Branch `mobile-authenticated-private-media` adds an authenticated media image component for mobile delivery-note and driver-uploaded photos. Source-level validation is in progress; preview APK and physical Android validation are still required before public R2 shutdown can be considered for owner approval. |
