...
| Mobile authenticated private media | Merge-Gate Passed; Render Deploy and Physical Validation Required | Branch `mobile-authenticated-private-media` adds authenticated private-media rendering and repairs the root mobile session/context model after physical Android testing exposed camera upload, camera-return login state, stop-scoped note retrieval, four-photo preview truncation, nondeterministic post-camera return state, photo-save non-persistence, and Driver Copilot authentication defects. Merge-gate validation passed for the backend `/api/ai` driver-auth repair, mobile session restoration, and photo persistence fixes. The backend repair must be deployed to Render and physically validated on Android before Driver Copilot acceptance or public R2 shutdown can be considered for owner approval. |
