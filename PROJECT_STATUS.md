...
| Mobile authenticated private media | Ready For Physical Validation | Branch `mobile-authenticated-private-media` adds authenticated private-media rendering and camera/library photo selection for driver-facing upload workflows. A new preview APK and physical Android validation are still required before public R2 shutdown can be considered for owner approval. |
