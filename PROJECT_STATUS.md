...
| Mobile authenticated private media | Forensic Source Repair Complete; Backend Deploy and Physical Validation Required | Branch `mobile-authenticated-private-media` adds authenticated private-media rendering and repairs the root mobile session/context model after physical Android testing exposed camera upload, camera-return login state, stop-scoped note retrieval, four-photo preview truncation, nondeterministic post-camera return state, photo-save non-persistence, and Driver Copilot authentication defects. The latest source repair centralizes session bootstrap before navigation workflows and prevents photo saves from reporting queued success without authoritative media persistence. A backend deploy, new preview APK, and physical Android validation are still required before public R2 shutdown can be considered for owner approval. |
